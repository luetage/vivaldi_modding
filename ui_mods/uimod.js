...

{
  ...
  function profileImage(image) {
    image.innerHTML =
      '<svg width="18" height="18" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg"><path d="M1523 1339q-22-155-87.5-257.5t-184.5-118.5q-67 74-159.5 115.5t-195.5 41.5-195.5-41.5-159.5-115.5q-119 16-184.5 118.5t-87.5 257.5q106 150 271 237.5t356 87.5 356-87.5 271-237.5zm-243-699q0-159-112.5-271.5t-271.5-112.5-271.5 112.5-112.5 271.5 112.5 271.5 271.5 112.5 271.5-112.5 112.5-271.5zm512 256q0 182-71 347.5t-190.5 286-285.5 191.5-349 71q-182 0-348-71t-286-191-191-286-71-348 71-348 191-286 286-191 348-71 348 71 286 191 191 286 71 348z"></path></svg>';
  ...
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    else if (changeInfo.url === `${settingsUrl}keyboard`)
      setTimeout(
...
!(function () {
  ...
  const e = {
    ...
    prefix: "Prefix",
    prefixdesc:
      "A unique prefix made up of the following characters: A-Z 0-9 _",
    saveprivate: "Save Private Windows",
  };
  let t = {};
  function i(e) {
    vivaldi.sessionsPrivate.getAll((i) => {
      const n = e ? "PRIV" : "",
        o = t.LONM_SESSION_AUTOSAVE_PREFIX + n,
        s = t.LONM_SESSION_AUTOSAVE_MAX_OLD_SESSIONS,
        a = i
          .filter((e) => 0 === e.name.indexOf(o))
          .sort((e, t) => e.createDateJS - t.createDateJS),
        r = o + Math.round(Date.now() / 1000);
      if (
        !(function (e) {
          return (
            /^[^\\\/:\*\?"<>\|]+$/.test(e) &&
            !/^\./.test(e) &&
            !/^(nul|prn|con|lpt[0-9]|com[0-9])(\.|$)/i.test(e)
          );
        })(r)
      )
        throw new Error("[Autosave Sessions] Cannot name a session as " + r);
      ...
      let d = a.length + 1,
        c = 0;
      for (; d > s; )
        vivaldi.sessionsPrivate.delete(a[c].name, () => {}), c++, d--;
    });
  }
  function n() {
    chrome.storage.local.get("LONM_SESSION_AUTOSAVE_LAST_WINDOW", (e) => {
      const t = e.LONM_SESSION_AUTOSAVE_LAST_WINDOW;
      window.vivaldiWindowId !== t
  ...
  function o() {
    chrome.storage.local.get("LONM_SESSION_AUTOSAVE_LAST_PRIV_WINDOW", (e) => {
      const t = e.LONM_SESSION_AUTOSAVE_LAST_PRIV_WINDOW;
      window.vivaldiWindowId !== t
        ? chrome.windows.getAll((e) => {
            e.find((e) => e.id === t) ||
              chrome.storage.local.set(
                {
                  LONM_SESSION_AUTOSAVE_LAST_PRIV_WINDOW:
  ...
  const s =
    "chrome-extension://mpognobbkildjkofajifpdfhcoklimli/components/settings/settings.html?path=general";
  function a() {
    const e = document.querySelector(
        ".vivaldi-settings .settings-content section"
      ),
      i = document.getElementById("lonmAutosaveSessionsSettings");
    if (e) {
      if (!i) {
        const i = document.createElement("section");
        (i.className = "setting-section"),
          (i.id = "lonmAutosaveSessionsSettings");
        const n = document.createElement("div");
        n.insertAdjacentHTML("beforeend", "<h2>Autosave Sessions Mod</h2>"),
          r.forEach((e) => {
            n.appendChild(
              (function (e) {
                const i = t[e.id],
                  n = document.createElement("div");
                n.className = "setting-single";
                const o = document.createElement("h3");
                if (
                  ((o.innerText = e.title), n.appendChild(o), e.description)
                ) {
                ...
                switch (
                  ...
                  case String:
                    ...
                    break;
                  case Boolean:
                    (s.type = "checkbox"), i && (s.checked = "checked");
                    break;
                  default:
                ...
                e.max && (s.max = e.max);
                e.min && (s.min = e.min);
                e.pattern && (s.pattern = e.pattern);
                return (
                  s.addEventListener(
                    "input",
                    function (e) {
                      "checkbox" === e.target.type
                        ? (t[this.id] = e.target.checked)
                        : (e.target.checkValidity(),
                          e.target.reportValidity() &&
                            "" !== e.target.value &&
                            (t[this.id] = e.target.value)),
  ...
  function d() {
    window.vivaldiWindowId
      ? chrome.windows.getCurrent((e) => {
          e.incognito ||
            chrome.storage.local.set(
              { LONM_SESSION_AUTOSAVE_LAST_WINDOW: e.vivaldiWindowId },
              () => {
                setInterval(
                  n,
                  60 * t.LONM_SESSION_AUTOSAVE_DELAY_MINUTES * 1e3
                );
              }
            ),
            t.LONM_SESSION_SAVE_PRIVATE_WINDOWS &&
              e.incognito &&
              chrome.storage.local.set(
                { LONM_SESSION_AUTOSAVE_LAST_PRIV_WINDOW: e.vivaldiWindowId },
                () => {
                  setInterval(
                    o,
                    60 * t.LONM_SESSION_AUTOSAVE_DELAY_MINUTES * 1e3
  ...
  !(function () {
    const e = r.reduce((e, t) => ((e[t.id] = t.default), e), {});
    chrome.storage.local.get(e, (e) => {
      (t = e), setTimeout(d, 500);
    });
  })();
})();
