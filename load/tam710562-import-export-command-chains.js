...
(async () => {
  ...
  const gnoh = {
    i18n: {
      getMessageName(message, type) {
        ...
        return (
          message.replace(/[^a-z0-9]/g, (i) => "_" + i.codePointAt(0) + "_") +
          "0"
        );
      },
      getMessage(message, type) {
        return (
          chrome.i18n.getMessage(this.getMessageName(message, type)) || message
        );
      },
    },
    uuid: {
    ...
    dialog(title, content, buttons = [], config) {
      ...
      function onClickCloseDialog(
        ...
        if (
          ...
          !document
            ...
            .closest('.dialog-custom[data-dialog-id="' + id + '"]')
        ) {
          closeDialog(true);
        }
      }
      ...
      const dialogHeader = this.createElement(
        "header",
        {
          class: "dialog-header",
        },
    ...
    alert(message, okEvent) {
      const buttonOkElement = this.object.merge(
  ...
  const urls = {
    quickCommands:
      "chrome-extension://mpognobbkildjkofajifpdfhcoklimli/components/settings/settings.html?path=qc",
    general:
  ...
  function highlightJson(element, text) {
    ...
    const jsonNumbers = [...text.matchAll(/-?\d+\.?\d*((e)\+\d+)?/gi)].map(
      (match) => {
    ...
    const jsonBooleans = [...text.matchAll(/false|true|null/gi)].map(
      (match) => {
        const range = new Range();
        range.setStart(element.firstChild, match.index);
        range.setEnd(element.firstChild, match.index + match[0].length);
  ...
  function createEditor(attribute = {}, parent, inner, options) {
    ...
    const editor = gnoh.createElement(
      "div",
      gnoh.object.merge(
        {
          class: "editor",
  ...
  async function showDialogImport(commandChainsText) {
    const buttonInputElement = gnoh.object.merge(
    ...
    const buttonPreviewElement = gnoh.object.merge(
      gnoh.constant.dialogButtons.submit,
      {
        label: langs.preview,
        disabled: true,
  ...
  async function showDialogExport(key) {
    ...
    const buttonCopyElement = gnoh.object.merge(
      gnoh.constant.dialogButtons.submit,
      {
        label: langs.copy,
        click: () => {
    ...
    const buttonExportElement = gnoh.object.merge(
      gnoh.constant.dialogButtons.submit,
      {
        label: langs.export,
        click: () => {
    ...
    const master = gnoh.createElement(
      "div",
      {
    ...
    const masterDetail = gnoh.createElement(
      "div",
      {
    ...
    const masterItems = gnoh.createElement(
      "div",
      {
        class: "master-items",
        tabindex: 0,
    ...
    commandList.forEach((command) => {
      ...
      const checkbox = gnoh.createElement(
        "input",
        {
          type: "checkbox",
    ...
    const masterToolbar = gnoh.createElement(
      "div",
      {
    ...
    const buttonToolbarExports = [
      {
        ...
        events: {
          click: () => {
            ...
            commandListChecked = commandList.reduce(
    ...
    const detail = gnoh.createElement(
      "div",
      {
        class: "detail",
      },
    ...
    const detailContent = gnoh.createElement(
      "div",
      {
  ...
  function createCommandChainItem(chain, index) {
    ...
    const chainedCommandItemTitle = gnoh.createElement(
      "div",
      {
    ...
    if (typeof chain.param !== "undefined") {
      const chainedCommandItemParameter = gnoh.createElement(
        "div",
        {
  ...
  chrome.runtime.onMessage.addListener((info, sender, sendResponse) => {
    if (info.type === messageType) {
      (async () => {
        ...
        if (
          (window && window.id === vivaldiWindowId) ||
          info.windowId === vivaldiWindowId
        ) {
          switch (info.action) {
            case "import":
            ...
            case "check":
              if (checkCommandChains(info.data)) {
                ...
                for (const commandChain of commandChains) {
                  try {
                    const commandChainOld = await getCommandChainByKey(
                      commandChain.key,
                    );
                    if (
                      JSON.stringify(commandChainOld) ===
                      JSON.stringify(commandChain)
                    ) {
                      status.installed++;
                    } else {
                      status.update++;
