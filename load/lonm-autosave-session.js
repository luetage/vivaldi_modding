...
(function autoSaveSessionsMod() {
  ...
  function autoSaveSession(isPrivate) {
    vivaldi.sessionsPrivate.getAll((allSessions) => {
      ...
      const autosavesOnly = allSessions.filter(
        (x) => x.name.indexOf(prefix) === 0
      );
      const oldestFirst = autosavesOnly.sort((a, b) => {
      ...
      let numberOfSessions =
        oldestFirst.length + 1; /* length + 1 as we have just added a new one */
      let oldestIndex = 0;
  ...
  function triggerAutosave() {
    chrome.storage.local.get("LONM_SESSION_AUTOSAVE_LAST_WINDOW", (data) => {
      ...
      chrome.windows.getAll((openWindows) => {
        const foundLastOpen = openWindows.find(
          (window) => window.id === lastOpenedWindow
        );
        if (foundLastOpen) {
  ...
  function modSettingsPage() {
    const settingSection = document.querySelector(
    ...
    settingsDiv.insertAdjacentHTML(
      "beforeend",
      "<h2>Autosave Sessions Mod</h2>"
    );
  ...
  function makeSettingElement(modSetting) {
    ...
    if (modSetting.max) {
      input.max = modSetting.max;
    }
    if (modSetting.min) {
      input.min = modSetting.min;
    }
    if (modSetting.pattern) {
      input.pattern = modSetting.pattern;
    }
  ...
  function init() {
    if (window.vivaldiWindowId) {
      chrome.windows.getCurrent((window) => {
        if (!window.incognito) {
          chrome.storage.local.set(
            ...
            () => {
              setInterval(
                triggerAutosave,
                CURRENT_SETTINGS["LONM_SESSION_AUTOSAVE_DELAY_MINUTES"] *
                  60 *
                  1000
              );
            }
        ...
        if (
          CURRENT_SETTINGS["LONM_SESSION_SAVE_PRIVATE_WINDOWS"] &&
          window.incognito
        ) {
          chrome.storage.local.set(
            {
